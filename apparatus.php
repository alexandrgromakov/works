...
class ApparatusApi
{
  ...
  public function execute($s_command)
  {
    ...
    for($i = 0;$i<4;$i++)
    {
      ...
      if($s_command===$s_command_original)
        break;// Некоторые команды ничего не возвращают. Кода ошибки в них быть не может. Ну и хорошо.

      $a_result = explode(';',$s_command);
      if(!$a_result[0]||$a_result[0]!=self::ERROR_UNKNOWN)
        break;// Первое значение в ответе - код ошибки. Если первое значение пустое, то - успех.
    }
  ...
  public function purchaseAdd(array $a_data)
  {
    $this->verify();
    $this->execute(
      'add_plu;'.$a_data['i_code'].';0;1;0;0;1;1;0.00;;'.iconv('utf-8','windows-1251',$a_data['s_title']).';1;'
    );
  }
